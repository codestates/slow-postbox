            ...
            <div className='signUp-section'>
              <div className='main-section'>
                <div className='section-email'>
                  ...
                  <button
                    className='send-authcode-button'
                    onClick={handleEmailVerification}
                  >
                    인증코드발송
                  </button>
                </div>
                <div className='section-confirm-email'>
                  {isConfirmEmail === 1 ? '이메일로 코드를 전송했습니다' : ''}
                  {isConfirmEmail === -1
                    ? '해당이메일로 가입된 계정이 있습니다.'
                    : ''}
                </div>
                ...
                <div className='section-auth'>
                  ...
                  <input
                    className='input-authcode'
                    name='verificationCode'
                    onChange={handleChange}
                  ></input>
                  <button className='check-auth' onClick={handleConfirmEmail}>
                    인증코드 확인
                  </button>
                </div>
                <div className='scetion-check-authcode'>
                  {emailCode === 1 ? '인증에 성공하였습니다' : ''}
                  {emailCode === -1 ? '인증번호가 다릅니다' : ''}
                </div>
                ...
                <div className='section-possible'>
                  {' '}
                  {isPossiblePassword === 1 ? '사용가능한 비밀번호입니다' : ''}
                  {isPossiblePassword === -1
                    ? '비밀번호는 숫자와 영문자 조합으로 10~15자리를 사용해야합니다.'
                    : ''}
                </div>
                ...
                <div className='section-confirm'>
                  {isConfirmPassword === 1 ? '비밀번호가 일치합니다' : ''}
                  {isConfirmPassword === -1
                    ? '비밀번호가 일치하지 않습니다.'
                    : ''}
                </div>
