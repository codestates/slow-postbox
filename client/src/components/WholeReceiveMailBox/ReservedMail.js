...
export default function ReservedMail() {
	...
	const getReservedData = async () => {
		...
		await axios.get(`${process.env.REACT_APP_SERVER_API}/mails/receivedReservation`, { params: { email, page } })
			.then((res) => {
	...
	const getReservedDataPage = async () => {
		axios.get(`${process.env.REACT_APP_SERVER_API}/mails/receivedReservation`, { params: { email, page } })
			.then((res) => {
				setData(res.data.data);
				setCount(res.data.count)
			})
