...
import { faEnvelope, faEnvelopeOpen, faSearch, faTrashAlt } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className='adminMail-container'>
      ...
      <table>
          ...
          <th>전송 날짜</th>
          <th>예약 날짜</th>
          <th className="delete"><span>삭제</span></th>
          {isLoding
          ? (
          ...
          : dummyData.map((el,id)=>{
            return (
              <MailList setConfirm={setConfirm} el={el} key={id}/>
            )
          })}
      ...
      {confirm?<ConfirmMail setModal={setModal} setConfirm={setConfirm}/>:""}
...
function MailList({el, setConfirm}) {
  return (
              <tr>
                <td>{el.id}</td>
                <td>{el.isRead ? <FontAwesomeIcon icon={faEnvelopeOpen}/> : <FontAwesomeIcon icon={faEnvelope} />}</td>
                <td>{el.writerEmail}</td>
                <td>{el.receiverEmail}</td>
                <td>{el.created_at.slice(0,10)}</td>
                <td>{el.reserved_at.slice(0,10)}</td>
                <td className="delete"><FontAwesomeIcon icon={faTrashAlt} onClick={()=>{setConfirm(true)}}/></td>
              </tr>
  )
}
